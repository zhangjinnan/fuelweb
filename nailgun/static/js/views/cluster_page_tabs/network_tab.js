    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        makeChanges: function(e) {
            ...
            this.model.get('networks').get(row.data('network-id')).set({
                ...
                network_size: e && this.$(e.currentTarget).parent().hasClass('cidr') && row.data('network-name') != 'fixed' ? Math.pow(2, 32 - parseInt(_.last($('.cidr input', row).val().split('/')), 10)) : parseInt($('.network_size select', row).val(), 10)
            });
        ...
        initialize: function(options) {
            ...
            if (!this.model.get('networks')) {
                ...
                this.model.get('networks').deferred.done(_.bind(function() {
                    ...
                    _.each(_.filter(this.model.get('networks').models, function(network) {return network.get('name') != 'fixed';}), function(network) {
                        var cidr = network.get('cidr');
                        network.set({network_size: Math.pow(2, 32 - parseInt(_.last(cidr.split('/')), 10))}, {silent: true});
                    });
                    this.getFixedAmount();
                    this.render();
                }, this));
